...
successful parses, and with varying lengths of matching input. The first element
of the tuple is the parse result, while the second =String= element is the
remaining, unprocessed input. This interface can be unexpected if you're used to
parsers either failing or succeeding with a single result.
