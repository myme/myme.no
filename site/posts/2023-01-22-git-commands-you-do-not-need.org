...
configuration. In other words, a /fetch/ followed by an /integration/[fn:2].

Since the integration can result in conflicts, git will only allow you to
integrate into the /current branch/. After all, conflicts must be resolved in
the worktree, and the worktree sees only the current branch (by definition).
...
your feature branch[fn:3]. One reason might be to avoid invalidating much of a
...
check it out first[fn:4]. As it happens, this exactly what ~git push~ does:
...
[fn:2] An /integration/ is the process of combining things together either
through a /merge/ or a /rebase/.

[fn:3] Diffing and cherry-pick can of course also be done towards remote refs,
but for the incremental builds case it's /very/ useful to update a branch
/before/ checking it out.

[fn:4] An alternative approach is to sidestep the problem by creating /another/
worktree specifically for updating the ~main~ branch, for example by using the
[[https://git-scm.com/docs/git-worktree][worktree]] functionality.
...
my ~dotfiles~ while the other parent [[https://github.com/doomemacs/doomemacs/commit/e96624926d724aff98e862221422cd7124a99c19][e96624926]] is the current[fn:5] ~HEAD~ from the
~doomemacs~ repo.
...
have any parents.
 ...
 1. The /real/ "[[https://github.com/myme/dotfiles/commit/61a3f80babec8c1339391462590dafe7ff30fe7f][Initial import]]" created at the beginning of time[fn:6].
...
2. Thanks to [[https://enpo.no/][a reader]] I was directed towards exactly the post I was after that
...
[fn:6] Yeah, yeah... Not that long ago, I know. But I did also track
...
In the end, a merge commit in ~git~ tracks a ~tree~[fn:7] -- like any other commit --
...
[fn:7] Remember that ~git~ operates on /snapshots/, not changes (aka patches).

[fn:8] By "passive" I'm talking about any branch that's currently not checked
